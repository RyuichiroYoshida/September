...
namespace September.Common
{
    ...
    public class ImtStateMachine<TContext, TEvent> where TContext : NetworkStateMachineOwner<TContext>
    {
        ...
        {
            // メンバ変数定義
            internal Dictionary<TEvent, int> transitionTable;
            ...
            /// ステートを更新するときの処理を行います
            /// </summary>
            protected internal virtual void OnNetworkFixedUpdate()
            {
            }
            ...
            {
                //  継承先でFixedUpdateNetworkを使ってほしくないのでsealedにする
            }
        }
        ...
        private State currentState => currentStateId >= 0 ? states[currentStateId] : null;
        private State nextState => nextStateId >= 0 ? states[nextStateId] : null;
        ...
        public ImtStateMachine(TContext context, params State[] states)
        {
            // 渡されたコンテキストがnullなら
            if (context == null)
            {
            ...
            for (int i = 0; i < states.Length; i++)
            {
                ...
                states[i].transitionTable = new Dictionary<TEvent, int>();
            }
        ...
        /// <exception cref="InvalidOperationException">ステートマシンは、既に起動中です</exception>
        /// <exception cref="InvalidOperationException">ステートクラスのインスタンスの生成に失敗しました</exception>
        public void AddTransition<TPrevState, TNextState>(TEvent eventId) where TPrevState : State where TNextState : State
        {
            ...
            var prevState = GetState<TPrevState>();
            ...
            // 遷移テーブルに遷移を設定する
            prevState.transitionTable[eventId] = nextState.stateId;
        }
        ...
        /// <exception cref="InvalidOperationException">ステートマシンは、既に起動中です</exception>
        /// <exception cref="InvalidOperationException">ステートクラスのインスタンスの生成に失敗しました</exception>
        public void SetStartState<TStartState>() where TStartState : State
        {
            ...
            //  次に処理するステートの設定をする
            nextStateId = GetState<TStartState>().stateId;
            //  最初のステートのEnter()を呼びたいので更新する
            Update();
        }
        #endregion
        ...
        public virtual bool SendEvent(TEvent eventId)
        {
            ...
            if (!currentState.transitionTable.TryGetValue(eventId, out nextStateId))
            {
                // 任意ステートからすらも遷移が出来なかったのなら
                if (!GetState<AnyState>().transitionTable.TryGetValue(eventId, out nextStateId))
                {
                    // イベントの受付が出来なかった
                    return false;
                }
        ...
        public virtual void Update()
        {
            ...
            try
            {
                ...
                if (nextStateId < 0)
                {
                    ...
                    currentState.OnNetworkFixedUpdate();
                }
        ...
        private TState GetState<TState>() where TState : State
        {
            ...
            throw new InvalidOperationException("ステートクラスのインスタンスの取得に失敗しました");
        }
        #endregion
    }
    #endregion
    ...
    /// </summary>
    /// <typeparam name="TContext">このステートマシンが持つコンテキストの型</typeparam>
    public class ImtStateMachine<TContext> : ImtStateMachine<TContext, int> where TContext : NetworkStateMachineOwner<TContext>
    {
        /// <summary>
        /// ImtStateMachine のインスタンスを初期化します
        /// </summary>
        /// <param name="context">このステートマシンが持つコンテキスト</param>
        /// <exception cref="ArgumentNullException">context が null です</exception>
        public ImtStateMachine(TContext context, params State[] states) : base(context, states)
        {
        }
    }
    #endregion
